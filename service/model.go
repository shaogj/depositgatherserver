...
// 2023.0323new
/*
func GenerateAccountBSC(curengine *xorm.Engine, cointype string, privkey string, pubkey string, pubkeyaddr string) error {
	enginewrite := curengine
	curGjcAccountKeyTb := new(models.GjcAccountKeyTb)
	//2018.080301==sgj update:
	//accountsql := "select * from gjc_account_key_tb"
	...
	curGjcAccountKeyTb.CreatedTime = time.Now().Unix()
	curGjcAccountKeyTb.CoinType = cointype
	rows, err := enginewrite.Table("bsc_account_key_mults").Insert(curGjcAccountKeyTb)
...
*/
// 1028,for wdc
func GenerateAccountBSC(curengine *xorm.Engine, walletType, walletId string, coinType string, strAddr string, strPriv string, strPub string, strPubHash string) error {
	...
	////time.Now().Unix()
	rows, err := curengine.Table(models.TableBSCAccount).Insert(curAccount)
	if err != nil {
		log.Error("GenerateBSCAccount(),Insert row is :%v,rowsnum is:%d,err is-:%v \n", curAccount, rows, err)
		return err

	}
	log.Info("GenerateBSCAccount(),Insert row success!,rec is :%v,rowsnum is:%d \n", curAccount, rows)
	return nil
}
